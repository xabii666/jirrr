...
typedef struct {
    void *target;           // Address of the hooked function
    uint32_t original[5];   // Original instructions (max 5 for alignment)
    uint32_t count;         // Number of instructions overwritten
    bool active;            // Whether this hook is currently active
} HookBackup;

static HookBackup __hook_backups[A64_MAX_BACKUPS];
static volatile int32_t __backup_index = -1;
...
public:
    A64HookInit() {
        ...
        memset(__hook_backups, 0, sizeof(__hook_backups));
...
A64_JNIEXPORT void *A64HookFunctionV(void *const symbol, void *const replace,
    ...
    // Allocate backup slot
    int32_t backup_idx = __atomic_increase(&__backup_index);
    if (backup_idx >= 0 && backup_idx < A64_MAX_BACKUPS) {
        __hook_backups[backup_idx].target = symbol;
        __hook_backups[backup_idx].active = true;
    }
    ...
    if (llabs(pc_offset) >= (mask >> 1)) {
        ...
        if (backup_idx >= 0 && backup_idx < A64_MAX_BACKUPS) {
            __hook_backups[backup_idx].count = count;
            memcpy(__hook_backups[backup_idx].original, original, count * sizeof(uint32_t));
        }
        ...
        if (__make_rwx(original, 5 * sizeof(uint32_t)) == 0) {
            ...
            *reinterpret_cast<int64_t *>(original + 2) = __intval(replace);
            ...
            __make_rx(original, 5 * sizeof(uint32_t));
        } else {
        ...
        if (backup_idx >= 0 && backup_idx < A64_MAX_BACKUPS) {
            __hook_backups[backup_idx].count = 1;
            __hook_backups[backup_idx].original[0] = *original;
        }
        ...
        if (__make_rwx(original, 1 * sizeof(uint32_t)) == 0) {
            ...
            __make_rx(original, 1 * sizeof(uint32_t));
        } else {
            A64_LOGE("mprotect failed with errno = %d, p = %p, size = %zu",
...
A64_JNIEXPORT bool A64RestoreHook(void *const symbol) {
    // Find the backup for this symbol
    for (int i = 0; i <= __backup_index && i < A64_MAX_BACKUPS; ++i) {
        if (__hook_backups[i].target == symbol && __hook_backups[i].active) {
            uint32_t *target = static_cast<uint32_t *>(symbol);
            uint32_t count = __hook_backups[i].count;
            
            A64_LOGI("Restoring hook at %p, %u instructions", symbol, count);
            
            // Make memory writable
            if (__make_rwx(target, count * sizeof(uint32_t)) != 0) {
                A64_LOGE("mprotect failed with errno = %d, p = %p, size = %zu",
                         errno, target, count * sizeof(uint32_t));
                return false;
            }
            
            // Restore original instructions
            memcpy(target, __hook_backups[i].original, count * sizeof(uint32_t));
            __flush_cache(target, count * sizeof(uint32_t));
            
            // Restore memory protection
            __make_rx(target, count * sizeof(uint32_t));
            
            // Mark as inactive
            __hook_backups[i].active = false;
            
            A64_LOGI("Hook restored successfully at %p", symbol);
            return true;
        }
    }
    
    A64_LOGE("No active hook found for symbol %p", symbol);
    return false;
}

A64_JNIEXPORT int A64RestoreAllHooks() {
    int restored = 0;
    
    A64_LOGI("Restoring all active hooks...");
    
    for (int i = 0; i <= __backup_index && i < A64_MAX_BACKUPS; ++i) {
        if (__hook_backups[i].active) {
            uint32_t *target = static_cast<uint32_t *>(__hook_backups[i].target);
            uint32_t count = __hook_backups[i].count;
            
            // Make memory writable
            if (__make_rwx(target, count * sizeof(uint32_t)) != 0) {
                A64_LOGE("mprotect failed for %p, errno = %d", target, errno);
                continue;
            }
            ...
            A64_LOGI("Restored hook at %p", target);
            restored++;
        }
    }
    
    A64_LOGI("Restored %d hooks", restored);
    return restored;
}
}
